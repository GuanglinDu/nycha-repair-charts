...
client = SODA::Client.new({ :domain => "data.cityofnewyork.us",
                            :app_token => ENV["OPEN_DATA_APP_TOKEN"] })
...
records.each do |record|
  record.each do |r|
    violation = Violation.find_or_create_by(
      ...
      apt_number:          r.apartment,
      zip_code:            r["zip"],
      approval_date:       r.approveddate, 
      current_status_date: r.currentstatusdate,
      current_status:      r.currentstatus,
      boro:                r.boro, 
      description:         r.novdescription[20..-1].downcase,
      boro_id:             r.boroid)
    ...
    elsif r.novdescription.downcase.include?("window")        ||
          r.novdescription.downcase.include?("missing glass") ||
          r.novdescription.downcase.include?("window guards")
    ...
    elsif r.novdescription.downcase.include?("defective faucets") ||
          r.novdescription.downcase.include?("hot water")         ||
          r.novdescription.downcase.include?("cold water")
